...
We recluster the cells using >300 <7k genes, >500UMI < 50k UMIs as cut-offs and explore the dataset. The resulting data object is the starting point for lineage subclusterings.\  
...
We apply t-SNE dimensionality reduction for visualization purposes. 43 clusters are recovered using a resolution parameter of 1.5 that were annotated using published expression patterns (Fig. \ref{fig:annottsne}).\
...
cluster.names <- c("enEp_SC1","enEp_SC2","ecEp_SC1","ecEp_SC2","i_SC","i_nb1","i_nb2","enEp_SC3","ecEp_nem1(pd)","i_smgc","i_nc_prog","enEp_foot","i_gmgc","i_nb3","ecEp_bat","ecEp_head","i_nc1","i_gc_nc_prog","i_nc2","i_zmg1","i_fmgl1","ecEp_bd","ecEp_nem2(mp)","i_fmgl2","enEp_head","i_nc3","i_nc4","enEp_tent","i_nb3","i_mgl1","i_nb4","i_nc5","i_nc6","enEp_nem1(pd)","i_nc7","i_mgl2","i_zmg2","db1","db2","unident","i_nc8","enEp_tent(pd)","db3")
...
#p <- FeaturePlot(ds.ds, c(hFind("t31900aep"),hFind("t34741aep"),hFind("t30697aep"),hFind("t4961aep"),hFind("t18356aep"),hFind("t8678aep"),hFind("t14102aep"),hFind("t20198aep")), cols.use=c("grey", "blue"), do.return=TRUE)
...
#p <- FeaturePlot(ds.ds, c(hFind("t31900aep"),hFind("t34741aep"),hFind("t30697aep"),hFind("t4961aep"),hFind("t13977aep"), hFind("t24244aep"), hFind("t12614aep"), hFind("t23162aep")), cols.use=c("grey", "blue"), do.return=TRUE)
...
## Doublet exclusion, cells that express male germline specific histones outside the germline
...
# jpeg(("SA_figures/SA02_figures/histones.jpeg"), width=300, height=150, units = "mm", res=300, bg ="white")
# FeaturePlot(object = ds.ds, features.plot = c("t11585aep|H2BL1_PSAMI","t38683aep|H10A_XENLA"), no.legend = TRUE, cols.use=c("grey", "blue"))
# dev.off()
...
#cells to be excluded
#> select.cells
# [1] "11-BU_CGGGCAATGCCA" "06-MA_ACAGCGGCCCTC" "06-MA_CCTTGGTCCAAC" "01-D1_TTCAAATTCACC" "01-P2_CCGCTTAAGGAN" "03-MA_GGGTACTCTTCC" "03-MA_CTGTGATGTGAN"
# [8] "03-MA_AATCATTGGAGG" "03-MA_TCCTTCGCTGGA" "03-MA_CTCAACCCTCGT" "06-MA_CTCAGCCGCATC" "06-MA_CTCGCCGCATCN" "06-MA_GTTCCTCGGCGG" "06-MA_GCTTCTTTGACA"
#[15] "06-MA_GCAGGAACTTCA" "06-MA_AGGATAACAGTG" "06-MA_CAGACTGAAGAC" "06-MA_CGGAAGACGCGA" "06-MA_GAAGCCCTGTTG" "06-MA_AATAGGTACTGT" "06-MA_TTGGAATTCACC"
#[22] "06-MA_GGTAGGTGAGAG" "06-MA_ACCATGTTGTTN" "06-MA_ACCAATGTTGTT" "06-MA_CGTCGTTGAAAT" "06-MA_ATTTTGTGGTAG" "06-MA_AGCGTCTATTGT" "06-MA_AATGGTACTGTN"
#[29] "06-MA_ATCGCGTGATCN" "06-MA_CAGCTGAAGACN" "06-MA_CGGAGACGCGAN" "06-MA_AGTACGACCTTG" "06-MA_GGTCGGTAACAT" "06-MA_TGGGTTATCGGG" "06-MA_AGGTAACAGTGN"
#[36] "06-MA_AGTCGACCTTGN" "06-MA_GGATTGAATCAG" "06-MA_GGTGGTGAGAGN" "06-MA_CGCAACGGAATC" "06-MA_CGCACGGAATCN" "06-MA_GCACCGGCCATC" "06-MA_ATCAGCGTGATC"
#[43] "06-MA_TGTTGCGTGCGG" "06-MA_CTGGGGAGCAGG" "06-MA_TCTGTTTCGAAG" "06-MA_CAATCATGACTT" "06-MA_TTCCCCCAGGAT" "06-MA_AAATACGCTTCA" "06-MA_CTAGCTTGTTTC"
#[50] "06-MA_CACCAATCTCCA" "06-MA_GACTCAACACCG" "06-MA_GCCTAATGGATG" "06-MA_CGTCTTGAGTTA" "06-MA_TACTTAGTTTAT" "06-MA_ACTAAGTCCTGT" "06-MA_GCGCTATCGTAT"
#[57] "06-MA_GGGTGGATTATA" "06-MA_AAGCATCCCGTT" "06-MA_TGGGGGTGCCCG" "06-MA_TCAGAGGCCCGA" "06-MA_CTGACCGAAACG" "06-KI_TAGCCTCGTTCT" "11-BU_GTGTAGGGGTCA"
#[64] "11-BU_GCATAGAACCTG" "06-MA_GTGGATGCAAAT" "06-MA_GTCCGCGATTCA"
...
# Get all cell ids associated with germ-line clusters i_mgl1 and i_mgl2
...
We cluster the cells as before. We tested PCs from 1:19 through 1:37 with three different seeds (1 (default), 100, 4024) and three perplexities, 20, 30 (default), 40. No principal components were excluded. The selected analysis used PCs 1:31 and a perplexity of 40 (seed=100) (Fig. \ref{fig:tsneclust}). \
...
## Save object
#saveRDS(ds.ds,"objects/Hydra_Seurat_Whole_Transcriptome.rds")) 
...
```{r annottsne2, fig.height = 9, fig.width = 9, message=F, warning=F, fig.keep='last', fig.cap= "Annotated t-SNE plot (final gene/UMI cut-offs, post initial doublet filtering). bat: battery cell, db: doublet, ecEP: ectodermal epithelial cell, enEP: endodermal epithelial cell, fmgl: female germ-line, gc: gland cell, gmgc: granular mucous gland cell, i: cell of the interstitial lineage, id: integration doublet, mgl: male germline, mp: multiplet, nb: nematoblast, nc: neuronal cell, nem: nematocyte, pd: suspected phagocytosis doublet, prog: progenitor, SC: stem cell, smgc: spumous mucous gland cell, tent: tentacle, zmg: zymogen gland cell."}

## Label clusters
# Store cluster numbering
ds.s1 <- StashIdent(object = ds.s1, save.name = "cluster_numbering")
...
cluster.names <- c("enEp_SC1","i_SC","ecEp_SC2","ecEp_SC3","enEp_SC2","i_nb1","enEp_SC3","enEp_head","ecEp_nem1(pd)","i_nc_prog","ecEp_head","enEp_foot","i_smgc2","i_nb2","ecEp_bat2(mp)","i_nc_gc_prog","i_gmgc","i_nem","i_zmg1","i_mgl","i_nb3","i_fmgl1","i_nc1","enEp_tent","i_fmgl2_nurse","i_nb4","ecEp_bd","ecEp_nem2(id)","i_nc2","i_nc3","i_nc4","i_nc5","i_nc6","enEp_nem1(pd)","i_smgc1","i_zmg2","i_nb4","ecEp_bat1(mp)","i_nc7","enEp_nem2(pd)","i_nc8","enEp_tent(pd)","db")

# Update names in Seurat object
ds.s1@ident <- plyr::mapvalues(x = ds.s1@ident, from = current.cluster.ids, to = cluster.names)

# Store state labels
ds.s1 <- StashIdent(object = ds.s1, save.name = "states")
...
TSNEPlot(object = ds.s1, do.return=T, do.label = T, no.legend=TRUE, label.size = 8, pt.size = 0.5)
...
# Load state labels
ds.s1 <- SetAllIdent(object = ds.s1, id = "states")
...
## Save object with metagene information
#saveRDS(ds.ds,"objects/Hydra_Seurat_Whole_Transcriptome.rds")) 
...
```{r statestats, message=F, fig.height = 20, fig.width = 4.5, out.height="80%", fig.align='center', fig.cap= "Median genes/UMIs per cell per state (all libraries)."}
...
```{r libstats, fig.height = 20, fig.width = 4.5, out.height="30%", fig.align='center', fig.show='asis', fig.cap= "Cells, median genes/UMIs per cell per libraries. Cells with >300 <7k genes and >500UMI <50k UMIs."}
...
Finally, we take a look at number of genes, UMIs and the contribution from each library (batch) (Figs. \ref{fig:tsnegenes}, \ref{fig:tsneUMIs}, \ref{fig:tsnebatch}).\

```{r tsnegenes, fig.height = 9, fig.width = 9, out.height="50%", fig.align='center', fig.show='asis', fig.cap= "t-SNE plot. Number of genes."}

#number of genes
FeaturePlot(object = ds.s1, features.plot = "nGene", cols.use=c("grey", "green"))

```

```{r tsneUMIs, fig.height = 9, fig.width = 9, out.height="50%", fig.align='center', fig.show='asis', fig.cap= "t-SNE plot. Number of UMIs."}

#number of umis
FeaturePlot(object = ds.s1, features.plot = "nUMI", cols.use=c("grey", "red"))

```

```{r tsnebatch, fig.height = 9, fig.width = 9, out.height="50%", fig.align='center', fig.show='asis', fig.cap= "t-SNE plot. Cells by library."}

#by batch
TSNEPlot(object = ds.s1, group.by="orig.ident")
...
```{r nGreen, warning=F, message=F, fig.height = 6, fig.width = 12, out.height="70%", fig.align='center', fig.show='asis', fig.cap= "Cells from neuronal libraries 12-. A) Annotated t-SNE plot - whole data set. B) Annotated t-SNE plot - subset of cells from neuronal libaries 12-."}
...
plot_grid(p1, p2, labels="AUTO", label_size = 30, align = "h", ncol=2)

```

\pagebreak

```{r ngreenstats, warning=F, message=F, fig.height = 20, fig.width = 4.5, out.height="70%", fig.align='center', fig.show='asis', fig.cap= "Composition of transgenic cell population from strain nGreen used in FACS."}
