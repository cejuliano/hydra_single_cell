...
tentacle.genes.wellexpressed <- names(which(tentacle.max.mean.spline > 0.5 | basal.disc.max.mean.spline > 0.5 | body.max.mean.spline > 0.5))
hypo.genes.wellexpressed <- names(which(hypo.max.mean.spline > 0.5 | basal.disc.max.mean.spline > 0.5 | body.max.mean.spline > 0.5))
...
font.size <- 0.16
pdf(paste0(main.path, "URD/Ectoderm/Ectoderm-Varying-Tentacle.pdf"), width=17, height=22)
...
# Generate the actual heatmap and save as a PDF.
font.size <- 0.25
pdf(paste0(main.path, "URD/Ectoderm/Ectoderm-Varying-Hypostome.pdf"), width=17, height=22)
gplots::heatmap.2(as.matrix(se), Rowv=reorder(h.sez, max(pt.wm)-pt.wm, agglo.FUN=median), Colv=F, dendrogram="none", col=cols, trace="none", density.info="none", key=F, cexCol=0.8, cexRow=font.size, margins = c(5,8), lwid=c(0.3,4), lhei=c(0.4, 4), labCol=NA)
