...
We load the full data set and extract ectodermal epithelial clusters. We consider cells that express more than 500 genes and include the battery cell clusters but omit body column clusters that are also positive for nematoblast/nematocyte expression (Fig. \ref{fig:ectosubset}). This subset was used to perfom non-negative matrix factorization (NMF) to identify metagenes expressed in ectodermal epithelial cells (SA07_nmf.rmd, NMF analysis ec_K76).\ 
...
```{r ectosclust, fig.height = 8, fig.width = 16, cache.lazy = FALSE, cache=TRUE, message=F, warning=F, fig.keep='last', fig.align='center', fig.cap= "Clustering without batch regression reveals batch effects in epithelial cells of the ectodermal subset. A) t-SNE plot with clusters labeled. B) t-SNE plot with cells labeled by batch. Cells from libraries 02- separate from cells from libaries 11-."}
...
# We consider cells with expressing a minimum of 500 genes
...
plot_grid(p1[[1]], p2[[1]], ncol=2, labels="AUTO", label_size = 20, align = "h")
...
```{r integrated2, fig.height = 8, fig.width = 16, message=F, warning=F, fig.keep='last', fig.align='center', fig.cap= "Integrated cells from two sets of libraries after exclusion of suspected endoderm/ectoderm doublets. A) t-SNE plot with clusters labeled. B) t-SNE plot with cells labeled by batch. l02: libraries 02-, l11: libraries."}
...
p1 <- TSNEPlot(ds.ec.ss, do.label = T, do.return = T, pt.size = 0.5)
p2 <- TSNEPlot(ds.ec.ss, do.return = T, pt.size = 0.5, group.by = "stim")
plot_grid(p1, p2, ncol=2, labels="AUTO", label_size = 20, align = "h")
...
We load the full data set, extract endodermal clusters but omit body column clusters that are also positive for nematoblast/nematocyte expression (Fig. \ref{fig:endosubset}). This subset was used to perfom non-negative matrix factorization (NMF) to identify gene expression modules expressed in endodermal epithelial cells (SA07_nmf.rmd, NMF analysis en_K40). We adjust the lower cut-offs for genes and UMIs to 500 and 2k, respectively, to consider only cells of high quality.\ 
...
#> select.cells
#[1] "11-PO_TTAAGTAGGGCG" "01-D1_CTGGCGCAGATC" "02-CO_GATGCAGTCATG" "02-P1_TACCCTTCTTAN" "02-P1_CCCCCCAGTGCC" "03-KI_ATGTGAGTTGCA" "03-KI_CAAGTATTCCCC" #"03-KI_ATTCGAGACGCG"
#[9] "06-FM_TTTCGGCGGTTC"
...
# Removed cells positive for metagenes en19 and en36 
ds.en.s1.tr <- SubsetData(object = ds.en, subset.name="en19", accept.high=0.2, subset.raw = TRUE)
ds.en.s1.tr <- SubsetData(object = ds.en, subset.name="en36", accept.high=0.2, subset.raw = TRUE)
# Save object for URD analysis
#saveRDS(ds.en.s1.tr,"objects/Hydra_Seurat_Endo.rds"))

```

```{r cluster2, cache.lazy = FALSE, cache=TRUE, results='hide', message=F, warning=F, results='hide', fig.keep='none'}
